        ...
        <section id="main" class="p-relative" role="main">
            ...
            <section id="content" class="container">
                    ...
                    <div class="block-area" id="text-input">
                        <h3 class="block-title">修改角色信息</h3>
                        
                        <div class="row">
                                {{ csrf_field() }}
                                <div class="col-lg-4 dividi">
                                    <span>角色名：</span>
                                    <input type="text" class="form-control m-b-10" placeholder="请输入角色名" name='name' value="{{ $ob->name }}">
                                    <span>角色：</span>
                                    @foreach($auth_name as $auth_name)
                                    <label class="checkbox-inline">
                                        <input class="userCheckbox" type="checkbox" @if(in_array($auth_name->auth_name,$ob->auth)) checked @endif><span>{{$auth_name->auth_name}}</span>
                                    </label>
                                    @endforeach
                                </div>
                                <div class="col-lg-12">
        ...
        <script>
            ...
            $('#submitButton').click(function(){
                ...
                $('.userCheckbox:checked').each(function(){
                    auth +=this.parentNode.nextSibling.innerHTML+',';
                });
                auth=auth.substring(0,auth.length-1);
                $.ajax({url:'/Admin/auth/role/edit',
                    type:'post',
                    data:{'id':{{$id}},"name":$("input[name='name']").val(),"auth":auth},
