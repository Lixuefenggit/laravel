...
<link rel="stylesheet" href="{{ asset('Home/mjc/yanxuanmoban/css/nav.css') }}">
</head>
<body>
...
<div class="content">
   <div class="row">
		<div class="nav_div"> 
				    @foreach($list as $k=>$v)
				    <a href="#{{ $k+1 }}">
				            <div style="float:left; width:70px; height:60px; margin:auto"><img src='{{ trim($v->img,".") }}' width='70' height='58'></div>  
	                        <span class="nav_name">{{ $v->navname }}</span>
	                </a>       
	                @endforeach	
	    </div>
    <div class="nav_content">
		<div class="nav_left">
			<div class="nav_left_b">
				@foreach($lista as $v)
				<div class="nav_left_c_img">
					<div class="nav_left_c_img_title">
						<div class="nav_left_c_img_title_titleimg"> 
							<img class="nav_imga" src="{{ asset('Home/mjc/yanxuanmoban/images/navimg_a.png') }}">
						</div>
							<div class="nav_titlea" id="1">
								<h3 class="name">发布人：{{ $v->title }}</h3>
								<p class="issuetime">发布时间：{{ $v->time }}</p>
							</div> 				
					</div>
					<div class="nav_imgb">
						<a href="/Home/Qtglcontenta/chaxun/{{$v->id}}" class="nav_imgb_href">
							<img class="nav_imgb"  src='{{ trim ($v->img,".") }}' width='560' height='320'>
							<h3 class="nav_imgb_href_imgtitle">{{ $v->img_title}}</h3>
							<p class="nav_imgb_href_imgcontent">{{ $v->img_content}}</p>
						</a>
					</div>
				</div>
				@endforeach
		...
		<div class="hotbox">
			<div class="hotbox_content">
				<h2 class="hotbox_content_h2title">热门专题</h2>
				<ul class="hotbox_content_ulcontent">
				@foreach($listhot as $v)
					<a class="hotbox_content_ulcontent_aonecontent">			
						<div class="hotbox_content_ulcontent_aonecontent_img">
							<img src='{{ trim ($v->img,".") }}' width='100' height='58'>
						</div>					
						<div class="hotbox_content_ulcontent_aonecontent_contentbox">
							<h4 class="hotbox_content_ulcontent_aonecontent_contentbox_title">{{ $v->imgtitle}}</h4>
							<p class="hotbox_content_ulcontent_aonecontent_contentbox_content">{{ $v->imgcontent}}</p>
						</div>
					</a>
				@endforeach		
				</ul>
