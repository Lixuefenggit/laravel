function genHead(cateAttr)
{
    // 生成表格
    var table=$("商品规格：<table width='800' border='2' id='conten-table' style='margin-left:80px'></table>");
    $('#selfSubmit').before(table);
    // 生成表头
    var tr="<tr>";
...
function genBody(cateAttr,cateAttrValue)
{
    // 获取表格行数
    function getRows(id)
    {
    ...
    for(var m=1;m<getRows('conten-table');m++){
        var price=$("<td><input type='text' class='priceInput' style='width:80px;color:black;margin:4px;background:#E6E6E6;font-weight:bold'></td>");
        var count=$("<td><input type='text' class='countInput' style='width:80px;color:black;margin:4px;background:#E6E6E6;'></td>");
